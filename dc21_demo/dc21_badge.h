...
class DC21_badge {
	public:
		...
		int stick_x(bool raw_data = false);  // returns the calibrated position 0-31 of the joystick left-right
		int stick_y(bool raw_data = false);  // returns the calibrated position 0-31 of the joystick up-down

		bool stickUp_debounced(unsigned int dtime);     // returns True for JOY_UP movement for more then 5ms
		bool stickDown_debounced(unsigned int dtime);   // returns True for JOY_DOWN movement for more then 5ms
		bool stickLeft_debounced(unsigned int dtime);   // returns True for JOY_LEFT movement for more then 5ms
		bool stickRight_debounced(unsigned int dtime);  // returns True for JOY_RIGHT movement for more then 5ms
		bool stickUp(unsigned int dtime);   // returns True for JOY_UP movement every [dtime]ms
		bool stickDown(unsigned int dtime); // returns True for JOY_DOWN movement every [dtime]ms
		
		
		// Joystick calibration - recieves limits
		void stickCalibration(unsigned int left, unsigned int right, unsigned int up, unsigned int down, unsigned int center_x, unsigned int center_y);
	...
	private:
		...
		unsigned int joy_u_max;    // max up position
		unsigned int joy_y_center;
		unsigned int joy_d_max;    // max down position
		unsigned int joy_l_max;    // max left position
		unsigned int joy_x_center;
		unsigned int joy_r_max;    // max right position
		
		long lastStickUp;    // stick up debounce counter
		long lastStickDown;  // stick down debounce counter
		long lastStickLeft;  // stick left debounce counter
		long lastStickRight; // stick right debounce counter
